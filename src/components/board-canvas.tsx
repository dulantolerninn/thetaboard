...
const Excalidraw = dynamic(() => import("@excalidraw/excalidraw").then((module) => module.Excalidraw), { ssr: false, loading: () => <div className="grid h-full place-items-center"><LoaderCircle className="animate-spin text-indigo-600" /></div> });
...
export function BoardCanvas({ board }: { board: Board }) {
  ...
  return <div className="flex h-[calc(100vh-4rem)] flex-col"><div className="flex h-14 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-5"><h2 className="font-semibold">{board.title}</h2><span className="flex items-center gap-1.5 text-xs text-slate-400">{state === "saving" ? <><Cloud size={15} /> Guardando…</> : <><Check size={15} className="text-emerald-600" /> Guardado</>}</span></div><div className="min-h-0 flex-1"><Excalidraw initialData={{ elements: board.scene?.elements as never[] ?? [] }} onChange={(elements) => save(elements)} /></div></div>;
}
